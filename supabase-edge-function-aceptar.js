// Supabase Edge Function para procesar aceptación de cotizaciones
// Archivo: supabase/functions/aceptar-cotizacion/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    await sendNotificationEmail(supabase, cotizacionData, clientInfo)
...
async function sendNotificationEmail(supabase, cotizacion, clientInfo) {
  try {
    // Aquí implementaremos el envío de email de notificación
    ...
    // Insertar log de aceptación
    await supabase
      .from('logs_aceptacion')
      .insert(logData)

    console.log('Cotización aceptada:', logData)
  } catch (error) {
